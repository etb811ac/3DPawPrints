...
try{
    ...
    $mail->Subject = 'Nueva cotizacion - ' . $data['name'];
    $mail->IsHTML(true);
    ...
    $mail->msgHTML('<h1>Has recibido una nueva cotizacion:</h1><p><strong>NOMBRE: </strong>' . $data['name'] . '</p><p><strong>CORREO: </strong>' . $data['email'] . '</p><p><strong>TELEFONO: </strong>' . $data['phone'] . '</p><p><strong>NOTAS: </strong>' . $data['notes'] . '</p> <h2>Lista de productos:</h2><table >' . $data['products'] .'</table>' , __DIR__);
